...
const GRID_DISTRIBUTIONS: GridDistribution[] = [
  "symmetric",
  "asymmetric",
  "minimal",
];
    ...
    test.describe(`Grid Distribution: ${distribution}`, () => {
      ...
      test(`Admin: Can configure ${distribution} grid`, async ({
        ...
        for (let i = 0; i < gridConfig.length; i++) {
          ...
          await expect(page.getByTestId(`grid-column-${i}-score`)).toHaveText(
            scoreText,
          );
          // Capacity is in a tooltip or hidden until hover, but we can check data-testid
          await expect(
            page.getByTestId(`grid-column-${i}-slots`),
          ).toBeVisible();
        }
      });

      test(`API: ${distribution} grid saves correctly`, async ({
        testDb,
        authToken,
      }) => {
        const gridConfig = testDataBuilders.gridConfig(distribution);
  ...
  test.describe("Grid Column Manipulation", () => {
    ...
    test("Admin: Can modify column capacity", async ({
      ...
      const initialSlots = await page
        ...
        .count();
      await page
        .locator('button[aria-label^="Increase capacity"]')
        .first()
        .click();

      const finalSlots = await page
        .getByTestId("grid-column-0-slots")
        .locator("div")
        .count();
