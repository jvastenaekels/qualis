...
test.describe('Admin Profile Management', () => {
  ...
  test('should verify sidebar profile link exists', async ({ page }) => {
    ...
    const userMenu = page.locator('button[data-testid="user-menu-trigger"], button:has-text("admin@example.com")').first();
  ...
  test('should validate password change requirements', async ({ page }) => {
    ...
    await expect(page.getByText('Failed to change password. check current password.')).toBeVisible();
  });
});
