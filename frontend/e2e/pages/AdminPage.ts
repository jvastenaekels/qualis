...
export class AdminPage extends BasePage {
  ...
  async createStudy(title: string, slug: string) {
    ...
    const createResponsePromise = this.page.waitForResponse(
      (resp) =>
        resp.url().includes("/api/admin/studies") && resp.status() === 201,
    );
  ...
  async configureQSort(statements: string[]) {
    ...
    await expect(
      this.page.getByText(/Forced distribution grid/i),
    ).toBeVisible();
  ...
  async launchStudy() {
    ...
    if (await activateBtn.isDisabled()) {
      ...
      for (const item of items) {
        ...
        const isComplete =
          (await item.locator("svg.text-green-500").count()) > 0;
        console.error(
    ...
    if (appeared) {
      ...
      const errorItems = this.page.locator(
        'div[role="dialog"] .bg-slate-50 span',
      );
      const errors = await errorItems.allTextContents();
      throw new Error(
        `Activation failed with validation errors: ${errors.join(", ")}`,
      );
    }
  ...
  async captureParticipantTable(name: string = "participant-table") {
    ...
    return await this.visual.captureElement(
