...
export class AdminPage extends BasePage {
    ...
    async waitForSync() {
        ...
        await expect(this.page.getByTestId('sync-status')).toContainText(/saved/i, { timeout: 15000 });
    }
    ...
    async launchStudy() {
        ...
        await this.page.evaluate(() => {
            const keys = Object.keys(localStorage).filter(k => k.startsWith('open-q-draft-backup-'));
        ...
        const discardBtn = this.page.getByRole('button', { name: /discard local version/i });
        ...
        if (await activateBtn.isDisabled()) {
            ...
            const progress = await this.page.getByTestId('checklist-progress').getAttribute('style');
        ...
        const errorDialogTitle = this.page.locator('div[role="dialog"] h2', { hasText: /Configuration Incomplete/i });
