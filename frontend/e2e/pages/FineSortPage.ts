...
export class FineSortPage extends BasePage {
  ...
  async verifyLayout() {
    ...
    await expect(this.page.getByRole("button", { name: /zoom/i })).toHaveCount(
      2,
    );
  ...
  async moveFirstCardToGrid() {
    ...
    const deckCard = this.deckContainer
  ...
  async verifyWorkbenchActive() {
    await expect(
      this.page.getByText(/tap grid to place|place on grid/i).first(),
    ).toBeVisible();
  }
