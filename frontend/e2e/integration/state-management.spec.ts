...
test.describe('State Management Flow Tests', () => {
    ...
    test.describe('Configuration Updates', () => {
        test('Configuration changes do not affect ongoing participant sessions', async ({
            ...
            const _originalLabel = await participantPage
        ...
        test('New participants get updated configuration', async ({
            ...
            await expect(participantPage.getByTestId('rough-agree-btn')).toContainText(
                'NEW AGREE LABEL'
            );
    ...
    test.describe('Data Persistence', () => {
        ...
        test('Study submission creates participant record', async ({ page, testDb, authToken }) => {
            ...
            await expect(page.locator('[data-testid^="slot_"] [data-testid^="card-"]')).toHaveCount(
                10
            );
