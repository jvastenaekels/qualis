...
test.describe("State Management Flow Tests", () => {
  test.describe("Study State Transitions", () => {
    ...
    test("In-progress participant session persists across page reloads", async ({
      page,
      testDb,
      authToken,
    }) => {
    ...
    test("Study state change in Admin reflects immediately in participant view", async ({
      ...
      await expect(adminPage).toHaveURL(
        new RegExp(`/admin/studies/${study.slug}`),
      );
      ...
      await expect(
        adminPage.getByRole("status").filter({ hasText: "Paused" }),
      ).toBeVisible();
  ...
  test.describe("Configuration Updates", () => {
    test("Configuration changes do not affect ongoing participant sessions", async ({
      context,
      testDb,
      authToken,
    }) => {
    ...
    test("New participants get updated configuration", async ({
      context,
      testDb,
      authToken,
    }) => {
      ...
      await expect(
        participantPage.getByTestId("rough-agree-btn"),
      ).toContainText("NEW AGREE LABEL");
  ...
  test.describe("Data Persistence", () => {
    test("Participant progress saved and recoverable", async ({
      page,
      testDb,
      authToken,
    }) => {
      const study = await testDb.createStudy(
        ...
        testDataBuilders.study({
          ...
          presort_config: {
            enabled: true,
            fields: {
              name: testDataBuilders.presortField("text", "Name", {
                required: true,
              }),
            },
          },
    ...
    test("Study submission creates participant record", async ({
      page,
      testDb,
      authToken,
    }) => {
      ...
      await expect(
        page.locator('[data-testid^="slot_"]').first(),
      ).toBeVisible();
      ...
      for (const pile of piles) {
        ...
        const currentDeckArea = page.locator(
          `[data-testid="deck-area-${pile}"]`,
        );

        // Drag all cards currently in this pile
        while (
          (await currentDeckArea.locator('[data-testid^="card-"]').count()) > 0
        ) {
          const card = currentDeckArea
      ...
      await expect(
        page.locator('[data-testid^="slot_"] [data-testid^="card-"]'),
      ).toHaveCount(10);
