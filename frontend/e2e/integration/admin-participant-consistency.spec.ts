...
test.describe("Admin → Participant Consistency Suite", () => {
  test.describe("Presort Configuration Consistency", () => {
    test("Presort fields configured in Admin appear in Participant flow", async ({
      ...
      await expect(page).toHaveURL(
        new RegExp(`/study/${study.slug}/rough-sort`),
      );
    });

    test("Disabled presort skips to rough sort", async ({
      page,
      testDb,
      authToken,
    }) => {
  ...
  test.describe("Q-Sort Grid Consistency", () => {
    test("Grid configuration in Admin matches Participant grid", async ({
      ...
      const totalCapacity = gridConfig.reduce(
        (sum, col) => sum + col.capacity,
        0,
      );
    ...
    test("Grid total capacity matches statement count", async ({
      page,
      testDb,
      authToken,
    }) => {
  ...
  test.describe("Post-Sort Configuration Consistency", () => {
    ...
    test("Email requirement enforced when enabled", async ({
      page,
      testDb,
      authToken,
    }) => {
  ...
  test.describe("Branding Consistency", () => {
    ...
    test.skip("Custom branding appears throughout participant journey", async ({
      ...
      await expect(
        page.locator('img[alt="Research Institute B"]'),
      ).toBeVisible();
    });
  });
  ...
  test.describe("Interface Customization Consistency", () => {
    ...
    test.skip("Custom UI labels appear in Participant interface", async ({
      ...
      await expect(page.getByTestId("rough-agree-btn")).toContainText(
        "Strongly Align",
        { ignoreCase: true },
      );
      await expect(page.getByTestId("rough-disagree-btn")).toContainText(
        "Strongly Oppose",
        { ignoreCase: true },
      );
      await expect(page.getByTestId("rough-neutral-btn")).toContainText(
        "Undecided",
        { ignoreCase: true },
      );
    });
