...
test.describe("Full Study Flow (Example Study)", () => {
  test.beforeEach(async ({ page }) => {
    // Mock Study API
    await page.route(
      `**/api/study/${mockStudyConfig.slug}**`,
      async (route) => {
        await route.fulfill({
          status: 200,
  ...
  test("should complete the full study lifecycle starting from Landing Page", async ({
    ...
    await expect(page).toHaveURL(
      new RegExp(`/study/${mockStudyConfig.slug}/welcome`),
    );
    await expect(
      page.getByRole("heading", { name: mockStudyConfig.title }),
    ).toBeVisible();
    ...
    await page
      .getByRole("button", { name: mockStudyConfig.ui_labels.start_button })
      .click();
    ...
    // Gender (Select)
    await page
      .getByLabel(/gender|genre|sukupuoli/i)
      .selectOption({ label: "Male" }); // 'Male' is English label
