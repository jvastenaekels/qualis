...
// ⟶ MOVE verbatim: `type RecorderState = …` (AudioRecorder.tsx:28) — exported.
export type RecorderState = 'idle' | 'recording' | 'stopped' | 'playing' | 'uploading';

// ⟶ MOVE verbatim: the AudioRecorderProps interface (AudioRecorder.tsx:8-26)
//   — renamed nothing; exported (the component will import it back).
...
// ⟶ MOVE verbatim: module-level cleanup helpers + the MutableRef type alias
//   (AudioRecorder.tsx:30-74). Pure utilities — module-private (not exported).
...
export interface UseAudioRecorderResult {
    ...
    // JSX-shell support: identifiers the moved body owns that the component's
    // JSX (Task 2) still references directly. Grouped to keep the role-based
    // shape; exposes only what exists (no fabricated handlers).
    ui: {
...
export function useAudioRecorder(props: AudioRecorderProps): UseAudioRecorderResult {
    // Wiring change (behaviour-identical): destructure `props` with the exact
    // identifiers the moved body references (the component used
    // `questionKey: _questionKey, …` — preserved here unchanged).
    const {
    ...
    // intact under tsc's noUnusedLocals.
    void _questionKey;
