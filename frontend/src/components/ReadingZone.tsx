...
const InlineIcon: React.FC<{
    ...
    <>
        ...
        />
        {showCodes && code && (
            <span className="text-indigo-400 font-bold text-2xs mr-1.5 align-middle">{code}</span>
        )}
    </>
);
...
const ReadingZone: React.FC<ReadingZoneProps> = ({ variant }) => {
    ...
    if (variant === 'overlay') {
        ...
        return (
            <div
                className={cn(
                    'absolute top-0 left-0 right-16 z-toolbar pointer-events-none transition-all duration-200 ease-in-out',
