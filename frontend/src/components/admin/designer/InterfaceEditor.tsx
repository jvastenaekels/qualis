        ...
        <div className="space-y-12 pb-12">
            ...
            <Card className="border-none shadow-sm bg-white rounded-2xl overflow-hidden">
                ...
                <CardContent className="space-y-6">
                    {[
                        [
                            { inputKey: 'welcome.start', tipKey: 'start' },
                            { inputKey: 'common.next', tipKey: 'next' },
                        ],
                        [
                            { inputKey: 'post.submit', tipKey: 'submit' },
                            { inputKey: 'fine.actions.validate', tipKey: 'confirm' },
                        ],
                    ].map((row, rowIdx) => (
                        <div
                            key={rowIdx}
                            ...
                            {row.map(({ inputKey, tipKey }) => (
                                <div key={inputKey} className="space-y-2.5">
                                            ...
                                            <TooltipProvider>
                                                <Tooltip>
                                                    ...
                                                    <TooltipContent className="text-2xs">
                                                        {t(
                                                            `admin.design.interface.nav.tooltips.${tipKey}`
                                                        )}
                                                    </TooltipContent>
                                                </Tooltip>
                                            </TooltipProvider>
                                    ...
                                    <Input
                                        ...
                                        onFocus={() => setActiveSubStep(inputKey)}
                                        placeholder={tPlaceHolder(inputKey)}
                                        disabled={readOnly}
                    ...
                    {[
                        {
                            ...
                            keyPrefix: 'common',
                            stances: [
                                { suffix: 'agree', termKey: 'agree' },
                                { suffix: 'neutral', termKey: 'neutral' },
                                { suffix: 'disagree', termKey: 'disagree' },
                            ],
                        },
                        {
                            sectionKey: 'grid',
                            keyPrefix: 'fine.legend',
                            stances: [
                                { suffix: 'agree', termKey: 'most_agree' },
                                { suffix: 'neutral', termKey: 'neutral' },
                                { suffix: 'disagree', termKey: 'most_disagree' },
                            ],
                        },
                    ].map((group, groupIdx) => {
                        const inputKeys = group.stances.map(
                            ({ suffix }) => `${group.keyPrefix}.${suffix}`
                        );
                        const anyOverridden = inputKeys.some((k) => uiLabels[k]);
                        return (
                            <div key={group.sectionKey}>
                                {groupIdx > 0 && <Separator className="bg-slate-100 mb-8" />}
                                <div className="space-y-6">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2 py-1.5 px-3 bg-slate-50 border border-slate-100 rounded-xl w-fit">
                                            <span className="text-2xs font-black text-slate-600">
                                                {t(
                                                    `admin.design.interface.terms.${group.sectionKey}`
                                                )}
                                            </span>
                                        </div>
                                        {anyOverridden && (
                                            <Button
                                                ...
                                                size="sm"
                                                disabled={readOnly}
                                                onClick={() => resetTerminologyGroup(inputKeys)}
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                                        {group.stances.map(({ suffix, termKey }) => {
                                            ...
                                            return (
                                                <div key={inputKey} className="space-y-2.5">
                                                    ...
                                                    <Input
                                                        ...
                                                        value={getLabel(inputKey)}
                                                        onChange={(e) =>
                                                            updateLabel(inputKey, e.target.value)
                                                        }
                                                        onFocus={() => setActiveSubStep(inputKey)}
                                                        placeholder={tPlaceHolder(inputKey)}
                                                        disabled={readOnly}
