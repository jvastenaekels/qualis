...
from app.routers.admin.users import create_user, list_users, delete_user
...
async def test_create_user_success(db: AsyncSession):
    ...
    result = await create_user(user_in=user_in, db=db, _admin=admin_user)
    
    assert result.email == "newuser@example.com"
    assert result.is_superuser is False
    
    # Verify in DB
    db_result = await db.execute(select(User).where(User.email == "newuser@example.com"))
...
async def test_create_duplicate_user_fails(db: AsyncSession):
    ...
    with pytest.raises(HTTPException) as excinfo:
        await create_user(user_in=user_in, db=db, _admin=admin_user)
...
async def test_delete_user_success(db: AsyncSession):
    ...
    await delete_user(user_id=target_user.id, db=db, current_user=admin_user)
...
async def test_delete_self_fails(db: AsyncSession):
    ...
    with pytest.raises(HTTPException) as excinfo:
        await delete_user(user_id=admin_user.id, db=db, current_user=admin_user)
