...
async def test_create_study_flow(
    client: AsyncClient, test_user: User, test_workspace: Workspace, db: AsyncSession
):
    """Test creating a study as an authenticated user with workspace access."""
    # 1. Login/Get Token
...
async def test_update_study_flow(
    ...
    # Setup: Token & Study exists (seed_study created 'test-study' in test_workspace (where test_user is admin))
