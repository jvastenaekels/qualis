...
| F-01-002 | Local `.env` file contains real database credentials | blocker | fixed | `8dbe635f` | `.env.example` committed at repo root; `.env` still gitignored; credential `xK7mQ9vR2pLw` confirmed never in git history (`git log -S` on the password returns only the audit doc itself). Recommendation (3) — pre-commit gitleaks hook — only partially landed (`.gitleaksignore` exists but no `gitleaks` entry in `.pre-commit-config.yaml` or CI); see notes below. |
