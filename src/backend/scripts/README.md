...
- **`src/backend/migrations/`**: Contains the actual Alembic migration versions and configuration.

### Managing Schema Changes

1. **Modify Models**: Update `src/backend/app/models.py`.
2. **Generate Migration**:
   ```bash
   make migration-new
   ```
   (This runs `alembic revision --autogenerate`)
3. **Review**: Check the generated file in `src/backend/migrations/versions/`.
4. **Apply**:
   ```bash
   make migrate
   ```
...
- **`init_db.py`**: (Located in `src/backend/`) Initializes the database from scratch. **WARNING**: Use `--reset` with caution as it wipes all data.
...
scalingo --app your-app run -- python src/backend/scripts/migrate.py
```
